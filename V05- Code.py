...
def view_receipt_page():
    ...
    def view_receipt():
        ...
        for x in cart:
            ...
            total = total + price
            Label(view_receipt_win, text=x + " - $" + str(price), font=("Arial", 13, "bold"), bg = L_BLUE).pack()

            Label(view_receipt_win, text=("------------"), bg = L_BLUE).pack()
            total_Label = Label(view_receipt_win, text="Total $" + str(total), font=("Arial", 13, "bold"), bg = L_BLUE, fg=NVY_BLUE)
            total_Label.place(relx = 0.10, rely= 0.75)
