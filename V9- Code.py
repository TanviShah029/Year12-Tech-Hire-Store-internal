...
def save_records():
    ...
    # Saving all new rrecords as well as the previous records of the customer.    
    new_record = {
...
def delete_records():
    ...
    lst_frame.pack(pady=5, padx=15, fill="both", expand=True)

    # I made a scrollbar so that it's easier for the user. 
    scrbr = Scrollbar(lst_frame)
    scrbr.pack(side="right", fill="y")

    listbox_records = Listbox(lst_frame, font=("Arial", 10), yscrollcommand=scrbr.set, selectmode="single")
    ...
    Button(delete_win, text="Delete", bg="red", fg=WHT, font=("Arial", 10, "bold"), width= 15, command=delete_details).pack(pady=10)
    Button(delete_win, text="Close", bg=NVY_BLUE, fg=WHT, width=17, command=delete_win.destroy).pack(pady=5) 
...
def accessories_return():
    ...
    current_win[0].protocol("WM_DELETE_WINDOW", close)
    ...
    def rtrn_search():
        if not etry_rtrn.winfo_viewable():
           ...
           print("Searching for user's entry:", etry_ret.get())
       
    etry_ret = Entry(rtrn_tool_bar)
    ...
    full_name_of_cust = Label(return_win_shown, text="Customer name: " + customer_info.get("first_name", "") + "" + customer_info.get("last_name", ""), bg=L_BLUE, fg=NVY_BLUE, font=("Arial", 13, "bold"), width = 30)
    full_name_of_cust.pack()
    full_name_of_cust.place(relx=0.25, rely=0.15)

    # Contact Details of the customer.
    cnt_details_of_usr = Label(return_win_shown, text="Contact Details:" + customer_info.get("contact_details", ""), font=("Arial", 13, "bold"), bg=L_BLUE, fg=NVY_BLUE, width =30)
    cnt_details_of_usr.pack()
    cnt_details_of_usr.place(relx=0.25, rely=0.33)

    # All buttons and a button container for return page.
    bar_btn_customer = Frame(return_win_shown, bg=L_BLUE)
    bar_btn_customer.pack(pady=(250,20))

    bar_btn_view_purchases= Button(bar_btn_customer, text="View purchases", bg=NVY_BLUE, fg=WHT, font=("Arial", 11, "bold"), width=15, height=2, command=view_receipt_page)
    bar_btn_view_purchases.pack(side="left", padx=2)

    bar_btn_return_item = Button(bar_btn_customer, text="Return item", bg=NVY_BLUE, fg=WHT, font=("Arial", 11, "bold"), width=15, height=2, command=accessories_return)
    bar_btn_return_item.pack(side="left", padx=2)

    bar_btn_receipt_no = Button(bar_btn_customer, text="Receipt number", bg=NVY_BLUE, fg=WHT, font=("Arial", 11, "bold"), width=15, height=2, command=view_receipt_page)
