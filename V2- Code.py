...
# name, picture
ACCESSORIES = {
    "Gaming Mouse": ["Gaming Mouse.png", 49],
    "Keyboard": ["Keyboard.png", 35],
    "Headsets": ["Headsets.png", 129],
    "Computer": ["Computer.png", 1089]
}
...
def load_data():
    if os.path.exists("Customer details.txt"):
        with open("Customer details.txt","r") as file:
            content=file.read()
            if content.strip() =="":
                return[]
...
def load_data():
    if os.path.exists("Customer details.json"):
        with open("Customer details.json","r") as file:
            content=file.read()
            if content.strip() =="":
                return[]
            return json.loads(content)
...
def computer_accessories():
    ...
    def show_computer_accessories(event=None):
        ...
        if choice in ACCESSORIES:
            try:
                ...
                image_box.config(text="no pic", image='')
    ...
    # Images show up here
    image_box = Label(cmpacc_win, bg="#e6f2ff")
...
def view_receipt_page():
    ...
    view_receipt_win.title("RECEIPT")
    view_receipt_win.geometry("600x550")
    ...
    rece_frame.pack(side="top", fill="x", ipady=3)

    # Main title for tech hire store
    rece_title = Label( view_receipt_win, text="RECEIPT", font=("Arial", 17, "bold"), bg=NVY_BLUE, fg=WHT)
    ...
    def gene_receipt_no():
        ...
        if not data:
            ...
            return max(r["receipt_no"]for r in data)+1
