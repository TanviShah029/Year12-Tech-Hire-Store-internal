...
def save_records():
    ...
    if len(contact_details_box.get()) != 10:
        errors.append("Contact details must be exactly 10 numbers")
...
def delete_records():
    ...
    for item in records:
        display = (
            ...
            "             | Receipt No: " + item.get("receipt_number", "N/A")
        )
...
def computer_accessories():
    ...
    cmpacc_win = Toplevel()
    cmpacc_win.title("Computer Accessories")
    ...
    # Logo image is shown here. 
    logo_image = Image.open("Images/logo.png")
    logo_image = logo_image.resize((55,55), Image.LANCZOS)
    logo_photo = ImageTk.PhotoImage(logo_image)
    ...
    # Function for adding accessories via quantity picker. 
    def add():
...
def view_receipt_page():
    ...
    def view_receipt():
     ...
     # All purchases shown in the cart.
        ...
        # Validation for barcode in receipt page. 
        if receipt_no != "N/A, (Save customer details first!)":
            try:
                ...
                # Image that is generated for the barcode. 
                img = Image.open("bar_code_rcpt.png")
                img = img.resize((240, 65), Image.Resampling.LANCZOS)
                img_bar_code = ImageTk.PhotoImage(img)
                
                # Label for barcode. 
                bar_cde_label = Label(view_receipt_win, image=img_bar_code, bg=L_BLUE)
                bar_cde_label.image = img_bar_code 
                bar_cde_label.place(relx=0.20, rely=0.83)
...
bar_btn_exit= Button(text="Exit", bg= NVY_BLUE, fg=WHT, font=("Arial", 10, "bold"), width = 17, height =2, command=cnfrm_exit_program)
...
def accessories_return():
    ...
    rtrn_tool_bar.pack(side="top", fill="x", ipady=4)

    # Logo image is shown here. 
    logo_image = Image.open("Images/logo.png")
    logo_image = logo_image.resize((55, 55))
    logo_photo = ImageTk.PhotoImage(logo_image)
    ...
    rtrn_tl = Label(rtrn_tool_bar, text="Byte and Bolt Tech Hire return", font=("Arial", 19, "bold"), bg=NVY_BLUE, fg=WHT)
    rtrn_tl.pack(anchor="center", padx=10, pady=4)
    ...
    # Function created for validating when customer tries to return with an empty cart.  
    def item_rtrn_cust(rcrds_list_of_customer, cust_idx_matched):
        ...
        def item_cnfrm_selection():
            ...
            # Counting the amount of items purchased and showing it. 
        ...
        # Buttons created for confirming return and closing window. 
    ...
    quit_btn = Button(return_win_shown, text="Quit", bg=NVY_BLUE, fg=WHT, font=("Arial", 11, "bold"), width=15, height=2, command=cnfrm_exit_program)
